...
## 自作 3D モデル（Blender）の差し替え

群れの小魚は、自作の .glb を置くだけでコード生成の魚から差し替わる。

手順:

1. Blender でモデルを作る。**約束事は3つだけ**
   - **頭を +X（赤軸）に向ける**。向きだけは自動で直せない
   - 原点・大きさは自由（読み込み時に中心へ寄せ、体長を 1 に自動正規化する）
   - **ボーン・アニメーション不要**。うねりは頂点シェーダーが後から掛ける
2. File → Export → glTF 2.0 で **.glb** として書き出し、`public/models/fish.glb` に置く
3. `public/models/manifest.json` の `"fish.glb"` を `true` にする
4. `npm run dev` で確認して push

注意:

- マテリアル・テクスチャは**無視される**。水中のライティング（カウンター
  シェーディング・リムライト・ベールの持ち上げ）はサイト側のシェーダーが
  掛けるので、Blender では形だけ作ればよい
- 三角形数は 1 万まで目安（インスタンス描画なので 48 匹でも 1 ドローコール）
- 複数メッシュでも可（読み込み時に 1 つへ結合される）
- 失敗しても壊れない。読めなければコード生成の魚のまま泳ぎ続ける
