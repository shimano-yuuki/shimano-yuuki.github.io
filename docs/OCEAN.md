...
3. **生き物** — `CreatureLayer.ts`（単独遊泳）+ `BoidsFlock.ts`（群れ）
4. **光条・カースティクス** — `oceanShaders.ts` の lightFragment。浅い層のみ
5. **気泡・マリンスノー** — 同 particleVertex/Fragment。1つの点群で両方
...
| 変えたいこと | 場所 |
|---|---|
| 水の流れの速さ・強さ | `FluidSimulation.ts` の `autoStir` 内 `stirrers`（speed / force） |
